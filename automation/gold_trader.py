#!/usr/bin/env python3
"""
Gold Futures Trader Dashboard — comprehensive monitoring for active gold futures traders.

Data sources:
1. Gold futures (COMEX GC=F) — price, technicals, volume
2. Gold ETFs (GLD, GDX, IAU, GLDM, SIL) — prices, flows proxy
3. Macro drivers: DXY, 10Y yield (^TNX), 30Y yield (^TYX), TLT, TIP, UUP
4. Technical indicators: RSI, MACD, Bollinger Bands, 50/200 DMA, ATR
5. CME FedWatch / Fed Funds futures implied path — rate-cut odds proxy
6. GLD options put/call ratio — positioning
7. IG retail sentiment — contrarian signal
8. Seasonal context
9. Polymarket — gold/fed related prediction markets

Output modes:
    python3 gold_trader.py --brief      # Slack-formatted briefing
    python3 gold_trader.py --json       # Raw JSON
    python3 gold_trader.py --levels     # Entry/exit levels only
    python3 gold_trader.py --full       # Everything (default)
"""
...
def get_session(now=None):
    """
    Identify the current gold-futures session in CME Globex hours (ET-based).
...
def fetch_intraday_last_price():
    """
    Last 1-minute bar for GC=F. Daily close is stale during a live session;
    this gives the most-current quote available via yfinance. Returns float
    or None if unavailable / off-hours.
    """
    try:
        h = yf.Ticker("GC=F").history(period="2d", interval="1m", auto_adjust=True)
...
CALENDAR_BY_YEAR = {
    2026: {
        ...
        "CPI":  [(1, 14), (2, 12), (3, 11), (4, 14), (5, 12), (6, 10), (7, 14), (8, 12),
...
def get_risk_events():
    ...
    for year, by_kind in CALENDAR_BY_YEAR.items():
        for kind, days in by_kind.items():
            event_name, impact, default_bias = EVENT_META.get(
    ...
    try:
        ...
        for item in cal_data:
            try:
                ...
                # Only grab HIGH impact events relevant to gold
                if impact != "HIGH":
                    continue
                ...
                if dt >= now - timedelta(days=1):
                    # Determine gold bias from event name
                    gold_bias = "uncertain"
                    title_upper = title.upper()
                    if any(w in title_upper for w in ["BOJ", "JAPAN"]):
                        gold_bias = "bearish"  # Yen strength = gold pressure
                    elif any(w in title_upper for w in ["OPEC", "OIL"]):
                        gold_bias = "bearish"  # Oil spike = inflation = possible rate hike
                    elif any(w in title_upper for w in ["FOMC", "FED"]):
                        gold_bias = "uncertain"
                    events.append({
                        "date": dt,
                        "event": title,
...
def fetch_futures_volume():
    """Fetch gold futures volume data."""
    try:
        gc = yf.Ticker("GC=F")
        hist = gc.history(period="30d", interval="1d", auto_adjust=True)
        if len(hist) == 0:
...
def fetch_gold_futures_term_structure():
    """Check gold futures contango/backwardation across contracts."""
    contracts = ["GC=F", "GCM26.CMX", "GCU26.CMX", "GCZ26.CMX", "GCG27.CMX"]
...
COT_CACHE_PATH = "/tmp/.gold_trader_cot_cache.json"
FRED_CACHE_PATH = "/tmp/.gold_trader_fred_cache.json"
FEDWATCH_CACHE_PATH = "/tmp/.gold_trader_fedwatch_cache.json"


def fetch_cot_data():
    """
    Fetch CFTC Commitments of Traders for Gold (Code 088691) — disaggregated futures only.
...
def fetch_real_yield():
    ...
    try:
        ...
        # Signal: real yield > 2% structurally bearish gold; falling = bullish
        if latest_val >= 2.0:
            level_signal = "ELEVATED (>2% — structurally bearish gold)"
        elif latest_val >= 1.5:
            level_signal = "MODERATE (1.5-2% — neutral)"
        elif latest_val >= 1.0:
            level_signal = "LOW (1-1.5% — supportive of gold)"
        else:
            level_signal = "VERY LOW (<1% — strongly bullish gold)"

        if change_1m <= -0.15:
            trend_signal = "FALLING (bullish gold)"
        elif change_1m >= 0.15:
            trend_signal = "RISING (bearish gold)"
        else:
            trend_signal = "STABLE"
...
def _fetch_fred_series(series_id, min_points=3):
    """Fetch a FRED CSV series without an API key. Returns [(date, value)]."""
    ...
    for row in text.strip().split("\n")[1:]:
        try:
            ...
            continue
    return parsed if len(parsed) >= min_points else []
...
def _pct_change(series, periods):
    if not series or len(series) <= periods:
        return None
    cur = series[-1][1]
    prev = series[-1 - periods][1]
    if prev == 0:
        return None
    return round((cur - prev) / prev * 100, 2)


def _abs_change(series, periods):
    if not series or len(series) <= periods:
        return None
    return round(series[-1][1] - series[-1 - periods][1], 2)


def fetch_labor_leading_indicators():
    """
    Leading labor-market proxies for NFP/Fed-path risk.

    Interpretation is gold-centric: softer labor is usually bullish gold because
    it pulls Fed pricing/real yields lower; hot wages are the bearish exception.
    Cached daily because the inputs are weekly/monthly releases.
    """
    cache_path = "/tmp/.gold_trader_labor_cache.json"
    today = _et_now().strftime("%Y-%m-%d")
    try:
    ...
    result = {"available": False, "score": 0, "signals": [], "indicators": {}}
    try:
        series = {
            "ICSA": _fetch_fred_series("ICSA"),          # initial claims, weekly
            "CCSA": _fetch_fred_series("CCSA"),          # continuing claims, weekly
            "UNRATE": _fetch_fred_series("UNRATE"),      # unemployment rate, monthly
            "TEMPHELP": _fetch_fred_series("TEMPHELPS"), # temporary help services, monthly
            "JOLTS": _fetch_fred_series("JTSJOL"),       # job openings, monthly
            "QUITS": _fetch_fred_series("JTSQUR"),       # quits rate, monthly
            "HOURS": _fetch_fred_series("AWHMAN"),       # avg weekly hours, manufacturing
            "WAGES": _fetch_fred_series("CES0500000003"),# avg hourly earnings, total private
            "ADP": _fetch_fred_series("ADPMNUSNERSA"),   # ADP private employment, monthly
        }
        ...
        # Weekly claims: compare latest 4w average with prior 13w baseline.
        for key, label in (("ICSA", "Initial claims"), ("CCSA", "Continuing claims")):
            s = series.get(key) or []
            if len(s) >= 17:
                latest = s[-1][1]
                avg4 = sum(v for _, v in s[-4:]) / 4
                avg13_prev = sum(v for _, v in s[-17:-4]) / 13
                delta = (avg4 - avg13_prev) / avg13_prev * 100 if avg13_prev else 0
                ind[key] = {"as_of": s[-1][0], "latest": int(latest), "avg4": int(avg4), "vs_13w_pct": round(delta, 1)}
                if delta >= 5:
                    add_signal(1, f"{label} rising {delta:.1f}% vs 13w baseline")
                elif delta <= -5:
                    add_signal(-1, f"{label} falling {abs(delta):.1f}% vs 13w baseline")

        # Monthly labor internals: 3m direction.
        checks = [
            ("UNRATE", "Unemployment", "up", 0.2, 1),
            ("TEMPHELP", "Temp help", "down", -1.0, 1),
            ("JOLTS", "Job openings", "down", -3.0, 1),
            ("QUITS", "Quits rate", "down_abs", -0.1, 1),
            ("HOURS", "Factory hours", "down_abs", -0.2, 1),
            ("ADP", "ADP private employment", "down", -0.15, 1),
        ]
        for sid, label, mode, threshold, points in checks:
            s = series.get(sid) or []
            if not s:
                continue
            chg = _abs_change(s, 3) if mode.endswith("abs") or sid == "UNRATE" else _pct_change(s, 3)
            ind[sid] = {"as_of": s[-1][0], "latest": round(s[-1][1], 2), "change_3m": chg}
            if chg is None:
                continue
            if mode == "up" and chg >= threshold:
                add_signal(points, f"{label} up {chg:+.2f} over 3m")
            elif mode == "down" and chg <= threshold:
                add_signal(points, f"{label} down {chg:+.2f}% over 3m")
            elif mode == "down_abs" and chg <= threshold:
                add_signal(points, f"{label} down {chg:+.2f} over 3m")

        # Wage pressure: bearish-gold offset if hourly earnings are accelerating.
        wages = series.get("WAGES") or []
        if len(wages) > 12:
            yryr = _pct_change(wages, 12)
            qtr = _pct_change(wages, 3)
            ind["WAGES"] = {"as_of": wages[-1][0], "latest": round(wages[-1][1], 2), "yoy_pct": yryr, "change_3m_pct": qtr}
            if yryr is not None and yryr >= 4.0:
                add_signal(-1, f"Wages still hot at {yryr:.1f}% YoY")
            elif yryr is not None and yryr <= 3.2:
                add_signal(1, f"Wages cooling at {yryr:.1f}% YoY")

        if score >= 3:
            bias = "SOFTENING — dovish/Fed-cut risk, bullish gold"
        elif score >= 1:
            bias = "MILD SOFTENING — modestly bullish gold into jobs data"
        elif score <= -2:
            bias = "FIRM LABOR — hawkish-payroll risk, bearish gold"
        elif score <= -1:
            bias = "MILDLY FIRM — slightly bearish gold"
        else:
            bias = "MIXED / NO EDGE"
        ...
        result = {
            ...
            "signals": signals[:6],
            "indicators": ind,
            "source": "FRED public CSV: ICSA/CCSA/UNRATE/TEMPHELPS/JTSJOL/JTSQUR/AWHMAN/CES0500000003/ADPMNUSNERSA",
        }
        with open(cache_path, "w") as f:
            json.dump({"fetched_on": today, "data": result}, f)
    except Exception as e:
        result["error"] = str(e)
    return result


def fetch_kalshi_macro_markets():
    """Read-only Kalshi public-market scan for macro/event contracts. No trading/auth."""
    out = {"available": False, "markets": [], "note": "No liquid macro markets found via public endpoint"}
    try:
        from urllib.parse import urlencode
        params = urlencode({"limit": 200, "status": "open"})
        url = f"https://api.elections.kalshi.com/trade-api/v2/markets?{params}"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        data = json.loads(urllib.request.urlopen(req, timeout=12).read().decode("utf-8", errors="ignore"))
        macro_keywords = ("payroll", "jobs", "unemployment", "jobless", "fed", "fomc", "cpi", "inflation", "treasury", "gdp")
        seen = set()
        markets = []
        for m in data.get("markets", []):
            text = " ".join(str(m.get(k, "")) for k in ("ticker", "title", "subtitle", "category", "event_ticker")).lower()
            category = str(m.get("category", "")).lower()
            # Public endpoint can be sports-heavy; avoid false positives like "Golden State" or "runs scored".
            if category in ("sports", "esports", "crypto") or "golden state" in text or "runs scored" in text:
                continue
            if not (any(k in text for k in macro_keywords) or re.search(r"\bgold\b", text)):
                continue
            ticker = m.get("ticker")
            ...
            prob = None
            if isinstance(yes_bid, (int, float)) and isinstance(yes_ask, (int, float)) and yes_bid and yes_ask:
                prob = round((yes_bid + yes_ask) / 2, 1)
            elif isinstance(yes_ask, (int, float)) and yes_ask:
                prob = yes_ask
            markets.append({
                "ticker": ticker,
        ...
        out = {"available": bool(markets), "markets": markets[:5]}
        if not markets:
            out["note"] = "Public scan returned no relevant/liquid macro contracts"
    except Exception as e:
        out["error"] = str(e)
    return out


def build_gold_catalyst_layer(risk_events, labor, macro, fedwatch, polymarket, kalshi):
    """Compact pre-catalyst read: event risk + leading indicators + priced-market layer."""
    now = _et_now()
    upcoming = []
    for e in risk_events or []:
        hours = (e["date"] - now).total_seconds() / 3600
        if 0 <= hours <= 24 * 7 and e.get("impact") == "HIGH":
            upcoming.append({**e, "hours_away": round(hours, 1)})
    upcoming.sort(key=lambda x: x["hours_away"])

    score = 0
    reasons = []
    labor_score = (labor or {}).get("score", 0)

    if upcoming and any(k in upcoming[0]["event"].upper() for k in ("NFP", "PAYROLL", "EMPLOY", "JOBS", "UNEMPLOY")):
        score += labor_score
        if (labor or {}).get("signals"):
            reasons.extend((labor or {}).get("signals", [])[:3])

    fw_score = (fedwatch or {}).get("score", 0)
    if fw_score:
        score += max(-2, min(2, int(fw_score)))
        reasons.append(f"Fed path: {(fedwatch or {}).get('signal', 'repricing')}" )

    if "FALLING" in (macro or {}).get("REAL_RATE_SIGNAL", "") or "FALLING" in (macro or {}).get("REAL_YIELD_TREND", ""):
        score += 1
        reasons.append("real yields falling")
    elif "RISING" in (macro or {}).get("REAL_RATE_SIGNAL", "") or "RISING" in (macro or {}).get("REAL_YIELD_TREND", ""):
        score -= 1
        reasons.append("real yields rising")

    if "WEAKENING" in (macro or {}).get("DOLLAR_SIGNAL", ""):
        score += 1
        reasons.append("DXY weakening")
    elif "STRENGTHENING" in (macro or {}).get("DOLLAR_SIGNAL", ""):
        score -= 1
        reasons.append("DXY strengthening")

    if score >= 3:
        bias = "BULLISH GOLD ASYMMETRY"
    elif score >= 1:
        bias = "MILD BULLISH GOLD EDGE"
    elif score <= -3:
        bias = "BEARISH GOLD EVENT RISK"
    elif score <= -1:
        bias = "MILD BEARISH GOLD RISK"
    else:
        bias = "NO CLEAR EVENT EDGE"

    return {
        "available": bool(upcoming or (labor or {}).get("available")),
        ...
        "prediction_markets": {
            "kalshi": kalshi,
            "polymarket": polymarket[:5] if polymarket else [],
        },
    }
...
def compute_technicals(gold_series, gold_ohlc=None):
    """
    Compute all technical indicators for gold futures.

    `gold_ohlc`, if provided, is a DataFrame with High/Low/Close columns
    (typically from yf.Ticker.history) and is used for true Wilder ATR.
    Without it, ATR falls back to a close-only approximation.
    """
    if gold_series is None or len(gold_series) < 50:
    ...
    atr_value = None
    atr_series = None
    if gold_ohlc is not None and all(c in gold_ohlc.columns for c in ("High", "Low", "Close")):
        try:
            atr_series = calc_atr(gold_ohlc["High"], gold_ohlc["Low"], gold_ohlc["Close"], period=14)
    ...
    result["KEY_LEVELS"] = _compute_key_levels(close, gold_ohlc, current, atr_value)
...
def _compute_key_levels(close, ohlc, current, atr):
    ...
    Combines: prev day H/L/C, prior week H/L, round numbers ($50 grid),
    ...
    # Round numbers: $50 grid spanning ±5 ATR around current
    if atr and atr > 0:
        span = max(150, atr * 5)
        lo = int((current - span) // 50) * 50
        hi = int((current + span) // 50 + 1) * 50
        for r in range(lo, hi + 1, 50):
            if r > 0:
                levels.append({"price": float(r), "type": "Round_$50"})
...
FED_FUNDS_MONTH_CODES = {
    1: "F", 2: "G", 3: "H", 4: "J", 5: "K", 6: "M",
    7: "N", 8: "Q", 9: "U", 10: "V", 11: "X", 12: "Z",
}


def _fetch_current_fed_target_range():
    """Fetch current Fed Funds target range from FRED; return (lower, upper)."""
    try:
        values = {}
        for series_id in ("DFEDTARL", "DFEDTARU"):
            ...
            for r in rows:
                try:
                    ...
                    continue
            if parsed:
                values[series_id] = parsed[-1]
        if "DFEDTARL" in values and "DFEDTARU" in values:
            return values["DFEDTARL"], values["DFEDTARU"]
    except Exception:
        return None
    return None


def _upcoming_fomc_dates(limit=4):
    """Upcoming FOMC dates from the local risk calendar."""
    now = _et_now()
    dates = []
    for year, cal in CALENDAR_BY_YEAR.items():
        for m, d in cal.get("FOMC", []):
            dt = _make_event_dt(year, m, d, "FOMC")
            if dt >= now:
                dates.append(dt)
    return sorted(dates)[:limit]


def fetch_fedwatch(num_meetings=4):
    """
    Fetch FedWatch-style rate-path data for gold macro scoring.

    CME's public FedWatch page is Cloudflare/403-prone from servers, so this
    routine uses Fed Funds futures prices (the same market CME FedWatch is
    derived from) as a robust fallback. It tracks the implied rate path and
    1w/1m repricing; lower implied rates / more cuts = bullish gold.
    """
    result = {
        "available": False,
        "source": "Fed Funds futures proxy (CME ZQ via Yahoo)",
        "meetings": [],
        "score": 0,
        "signal": "UNAVAILABLE",
    }

    # Cache for short-lived cron reliability; FedWatch data doesn't need tick precision.
    try:
        if os.path.exists(FEDWATCH_CACHE_PATH):
            with open(FEDWATCH_CACHE_PATH) as f:
                cached = json.load(f)
            ts = datetime.fromisoformat(cached.get("timestamp", ""))
            if (_et_now() - ts).total_seconds() < 60 * 60 and cached.get("available"):
                return cached
    except Exception:
        pass

    try:
        target = _fetch_current_fed_target_range()
        if target:
            lower, upper = target
            current_mid = (lower + upper) / 2.0
            result["target_range"] = f"{lower:.2f}-{upper:.2f}"
            result["target_mid"] = round(current_mid, 3)
        else:
            # FRED can hang. Fall back to the front Fed Funds futures implied rate
            # as the current policy-rate anchor so the signal still works.
            front = yf.Ticker("ZQ=F").history(period="5d", interval="1d", auto_adjust=True)
            close = front["Close"].dropna() if front is not None and "Close" in front else []
            if len(close) == 0:
                result["error"] = "Fed target range and front ZQ unavailable"
                return result
            current_mid = 100.0 - float(close.iloc[-1])
            result["target_range"] = "front ZQ proxy"
            result["target_mid"] = round(current_mid, 3)
            result["target_source"] = "front Fed Funds futures proxy"

        meetings = _upcoming_fomc_dates(limit=num_meetings)
        if not meetings:
            result["error"] = "No upcoming FOMC dates in local calendar"
            return result

        repricing_1w = []
        repricing_1m = []
        for dt in meetings:
            code = FED_FUNDS_MONTH_CODES[dt.month]
            symbol = f"ZQ{code}{str(dt.year)[-2:]}.CBT"
            try:
                hist = yf.Ticker(symbol).history(period="45d", interval="1d", auto_adjust=True)
                ...
                implied_rate = 100.0 - latest_price
                meeting = {
                    "date": dt.date().isoformat(),
                    "contract": symbol,
                    "price": round(latest_price, 3),
                    "implied_rate": round(implied_rate, 3),
                    "expected_change_bp": round((implied_rate - current_mid) * 100, 1),
                    "cuts_priced": round(max(0.0, (current_mid - implied_rate) / 0.25), 2),
                    "hikes_priced": round(max(0.0, (implied_rate - current_mid) / 0.25), 2),
                }
                if len(close) >= 6:
                    rate_1w = 100.0 - float(close.iloc[-6])
                    chg_1w = (implied_rate - rate_1w) * 100
                    meeting["repricing_1w_bp"] = round(chg_1w, 1)
                    repricing_1w.append(chg_1w)
                if len(close) >= 22:
                    rate_1m = 100.0 - float(close.iloc[-22])
                    chg_1m = (implied_rate - rate_1m) * 100
                    meeting["repricing_1m_bp"] = round(chg_1m, 1)
                    repricing_1m.append(chg_1m)
                result["meetings"].append(meeting)
            except Exception:
                continue

        if not result["meetings"]:
            result["error"] = "No ZQ futures data available"
            return result

        avg_1w = sum(repricing_1w) / len(repricing_1w) if repricing_1w else 0.0
        avg_1m = sum(repricing_1m) / len(repricing_1m) if repricing_1m else 0.0
        far = result["meetings"][-1]
        cuts_by_far = far.get("cuts_priced", 0.0)
        hikes_by_far = far.get("hikes_priced", 0.0)

        score = 0
        # Negative repricing = lower implied rates = dovish = bullish gold.
        if avg_1w <= -8:
            score += 2
        elif avg_1w <= -3:
            score += 1
        ...
        if cuts_by_far >= 1.0 and avg_1w <= 2:
            score += 1
        elif hikes_by_far >= 0.5:
            score -= 1

        score = max(-3, min(3, score))
        if score >= 2:
            signal = "DOVISH_REPRICING (bullish gold)"
        elif score == 1:
            signal = "MILD_DOVISH (bullish gold)"
        elif score <= -2:
            signal = "HAWKISH_REPRICING (bearish gold)"
        elif score == -1:
            signal = "MILD_HAWKISH (bearish gold)"
        else:
            signal = "NEUTRAL"
        ...
        result.update({
            ...
            "avg_repricing_1w_bp": round(avg_1w, 1),
            "avg_repricing_1m_bp": round(avg_1m, 1),
            "score": score,
            "signal": signal,
        })
        try:
            with open(FEDWATCH_CACHE_PATH, "w") as f:
...
def compute_macro(price_data, real_yield=None, fedwatch=None):
    """Analyze macro drivers of gold."""
    result = {}
    ...
    if tlt.get("price") and tip.get("price"):
        ...
        if tlt.get("returns", {}).get("1m") and tip.get("returns", {}).get("1m"):
            ...
            proxy_signal = "FALLING (bullish gold)" if tip_1m > tlt_1m else "RISING (bearish gold)"
    ...
    if dxy.get("price"):
        ...
        result["DOLLAR_SIGNAL"] = "WEAKENING (bullish gold)" if dxy.get("change_pct", 0) < 0 else "STRENGTHENING (bearish gold)"
    ...
    if tyx.get("price"):
        ...
        if tnx.get("price"):
            ...
            result["CURVE_SIGNAL"] = "STEEPENING (risk-on, mixed gold)" if result["YIELD_CURVE"] > 0 else "INVERTED/FLAT (risk-off, bullish gold)"
    
    # Miners leverage
    gdx = price_data.get("GDX", {})
    gld = price_data.get("GLD", {})
    if gdx.get("price") and gld.get("price"):
        result["GDX_GLD_RATIO"] = round(gdx["price"] / gld["price"], 4)
        if gdx.get("returns", {}).get("1m") and gld.get("returns", {}).get("1m"):
            result["MINER_LEVERAGE"] = round(gdx["returns"]["1m"] / gld["returns"]["1m"], 2) if gld["returns"]["1m"] != 0 else None

    # Gold/silver ratio — classic macro tell. >85 = fear trade (bullish gold relative);
    # <70 = risk-on (silver outperforms). Mid ranges are noise.
    ...
    if gc.get("price") and si.get("price") and si["price"] > 0:
        ...
        if gsr > 90:
            result["GSR_SIGNAL"] = "EXTREME (>90) — fear trade, gold-favored"
        elif gsr > 85:
            result["GSR_SIGNAL"] = "ELEVATED (>85) — defensive bias, gold-favored"
        elif gsr < 70:
            result["GSR_SIGNAL"] = "RISK-ON (<70) — silver outperforms, gold lagging"
        elif gsr < 75:
            result["GSR_SIGNAL"] = "MILD RISK-ON (<75)"
        else:
            result["GSR_SIGNAL"] = "NEUTRAL (75-85)"

    # FedWatch / Fed Funds futures implied rate path. Lower implied rates and
    # dovish repricing are bullish gold because they pressure real yields/USD.
    if fedwatch and fedwatch.get("available"):
        result["FEDWATCH"] = fedwatch
        result["FEDWATCH_SIGNAL"] = fedwatch.get("signal")
        result["FEDWATCH_SCORE"] = fedwatch.get("score", 0)
        meetings = fedwatch.get("meetings", [])
        if meetings:
            first = meetings[0]
            far = meetings[-1]
            result["FEDWATCH_NEXT"] = {
                "date": first.get("date"),
                "implied_rate": first.get("implied_rate"),
                "expected_change_bp": first.get("expected_change_bp"),
                "repricing_1w_bp": first.get("repricing_1w_bp"),
            }
            result["FEDWATCH_PATH"] = {
                "through_date": far.get("date"),
                "cuts_priced": far.get("cuts_priced"),
                "hikes_priced": far.get("hikes_priced"),
                "avg_repricing_1w_bp": fedwatch.get("avg_repricing_1w_bp"),
            }

    return result


def fetch_polymarket_gold():
    """Check Polymarket for gold/fed related markets."""
    result = []
    try:
        # Use the search endpoint instead of slug_contains
        search_terms = ["gold price", "federal reserve", "interest rate"]
        seen = set()
        ...
        for term in search_terms:
            ...
            for event in events:
                ...
                # Only include gold/fed/rate relevant markets
                if not any(kw in title.lower() for kw in ["gold", "fed", "rate", "treasury", "dollar", "powell", "inflation", "cpi"]):
                    continue
...
def fetch_gld_options(num_expiries=3, atm_band=0.08):
    """
    Fetch GLD options put/call ratios using yfinance option_chain.
    ...
    try:
        ...
        hist = gld.history(period="2d")
        if len(hist) == 0:
            result["error"] = "No GLD history"
            return result
        ...
        expiries = list(gld.options or [])[:num_expiries]
        if not expiries:
...
def fetch_ig_sentiment():
    """
    IG retail positioning for gold.
    ...
    try:
        url = "https://www.ig.com/uk/commodities/markets-commodities/gold"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        resp = urllib.request.urlopen(req, timeout=15)
...
def seasonal_context():
    """Return seasonal tendencies for gold based on current month."""
    month = _et_now().month
    
    # Historical gold seasonal patterns (based on 20yr average)
    seasonal = {
        1:  ("Neutral-Positive", "Jan tends to open strong — new money flows"),
        2:  ("Positive", "Feb historically one of strongest months"),
        ...
        8:  ("Strong Positive", "Aug-Sept historically BEST months for gold"),
...
CONTRACT_OZ = 10  # MGC (Micro Gold) contract size — 10 oz/contract on COMEX
...
def compute_conviction(technicals, macro, options, ig, fed_sentiment, cot=None):
    ...
    # GSR: extreme = gold-favored, risk-on = gold-lagging
    gsr = macro.get("GSR")
    if gsr:
        if gsr > 90:
            macro_sub += 1
            reasons.append("GSR extreme (fear trade)")
        elif gsr < 70:
            macro_sub -= 1
            reasons.append("GSR low (risk-on)")

    fedwatch_score = macro.get("FEDWATCH_SCORE", 0)
    if fedwatch_score:
        macro_sub += int(fedwatch_score)
        if fedwatch_score > 0:
            reasons.append(f"FedWatch dovish repricing ({fedwatch_score:+d})")
        else:
            reasons.append(f"FedWatch hawkish repricing ({fedwatch_score:+d})")

    macro_sub = max(-3, min(3, macro_sub))
    components["macro"] = macro_sub
    score += macro_sub

    # --- Positioning (up to ±2) ---
    pos_sub = 0
    oi_signal = options.get("signal", "NEUTRAL")
    if oi_signal in ("BULLISH", "MILDLY_BULLISH"):
        pos_sub += 1
        reasons.append("GLD options bullish")
    elif oi_signal == "BEARISH_HEDGE":
        pos_sub -= 1
        reasons.append("Heavy downside hedging")
...
def generate_levels(price, technicals, conviction, macro=None, options=None, ig=None):
    """Generate actionable entry/exit levels for gold futures."""
...
def generate_ladder(current_price, entry_price, contracts, max_contracts, technicals, levels,
    ...
    # Compute hard stop
    per_oz_loss = max_loss / (contracts * 10)
    ...
    if pnl_per_oz > 5 and contracts >= 2:
        ...
        if show_sell:
            ...
            tp_pnl = round((tp_price - entry_price) * 10 * tp_contracts, 0)
            ...
            if nearest_resistance and nearest_resistance > current_price and sell_remaining > 2:
                if conviction not in ("strong_bullish", "bullish"):
                    ...
                    r_pnl = round((nearest_resistance - entry_price) * 10 * r_contracts, 0)
                    sell_tiers.append({
...
def fetch_fed_news():
    ...
    try:
        proc = subprocess.run(
            ["ddgs", "news", "-k", "federal reserve gold interest rate Powell", "-m", "5", "-t", "d", "--no-color"],
    ...
    for url in fallback_urls:
        try:
            ...
            resp = urllib.request.urlopen(req, timeout=15)
            html = resp.read().decode("utf-8", errors="ignore")
            # Extract headlines that mention fed/rate/gold/dollar
            headlines = re.findall(r'<a[^>]*href="([^"]*)"[^>]*>([^<]*(?:Fed|rate|gold|dollar|Powell|Treasury|inflation|central bank)[^<]*)</a>', html, re.IGNORECASE)
...
NEWS_CACHE_PATH = "/tmp/.gold_trader_news_cache.json"
...
def _llm_classify_batch(items):
    ...
    prompt = f"""Classify each news item by its implication for GOLD prices.

- "dovish" = bullish gold (rate cuts, easing, recession risk, dollar weakness, soft landing concerns)
- "hawkish" = bearish gold (rate hikes, tightening, hot inflation forcing more hikes, dollar strength)
...
def classify_fed_news_sentiment(news_items, use_llm=True):
    """
    Classify Fed/macro news for gold-price implication.

    Returns:
        {
          "items": [news items annotated with label/confidence/reason],
          "dovish": int, "hawkish": int, "neutral": int,
          "net_score": int (dovish - hawkish; positive = bullish gold),
...
def format_brief(data):
    ...
    gold = data.get("price_data", {}).get("GC=F", {})
    gld = data.get("price_data", {}).get("GLD", {})
    gdx = data.get("price_data", {}).get("GDX", {})
    ...
    catalyst_layer = data.get("catalyst_layer", {})
    ...
    lines.append(f"🥇 **GOLD FUTURES TRADER BRIEFING** — {now_pt} ({now_et}){session_tag}")
    ...
    if gold.get("price"):
        ...
        intraday_tag = " (intraday)" if gold.get("intraday") else ""
        lines.append(f"{emoji} **GC=F: ${gold['price']}**{intraday_tag} ({chg:+.2f}%) | GLD: ${gld.get('price', '?')} | GDX: ${gdx.get('price', '?')}")
    ...
    if entry_price and gold.get("price"):
        current = gold["price"]
        ...
        per_contract_pnl = pnl * 10  # 10 oz per MGC (micro gold) contract
        total_pnl = per_contract_pnl * contracts if contracts else per_contract_pnl
        if contracts:
            lines.append("")
            lines.append(f"**💼 POSITION ({contracts} contracts MGC, avg entry: ${entry_price:.2f}):**")
        ...
        if "WEAKENING" in dxy_signal:
            macro_bullish += 1
            macro_signals.append("DXY weakening (bullish gold)")
        elif "STRENGTHENING" in dxy_signal:
            macro_bearish += 1
            macro_signals.append("DXY strengthening (bearish gold)")

        # Real rates
        rate_signal = macro_local.get("REAL_RATE_SIGNAL", "")
        if "FALLING" in rate_signal:
            macro_bullish += 1
            macro_signals.append("Real rates falling (bullish gold)")
        elif "RISING" in rate_signal:
            macro_bearish += 1
            macro_signals.append("Real rates rising (bearish gold)")
        ...
        # ---- RECOMMENDATION ----
        # Core thesis: gold goes up long-term. Default is HOLD. But:
        ...
        if imminent_risk and pnl_pct > 0.5 and hours_to_event and hours_to_event < 48:
            ...
            tp_profit = round((tp - entry_price) * 10 * contracts, 0)
            ...
            rec_lines.append(f"   Worst case: event is bullish and gold gaps up. You miss a move but protect ${total_pnl:,.0f}.")
            rec_lines.append(f"   Best case: event dumps gold, you re-enter ${round(current - re_entry, 0):.0f}/oz lower.")
        # ---- TACTICAL EXIT: chart deteriorating + bearish news ----
            ...
            rec_lines.append(f"   Set re-entry limit at **${re_entry:.0f}**. Gold always recovers — buy back at the floor, not on the way down.")
            ...
            if pnl_pct > 0:
                ...
                rec_lines.append(f"   Gold pulls back 10-15% in bull markets. You're not there yet. Hold.")
            else:
            ...
            # Below SMA_200 but macro says gold — buying opportunity
            if pnl_pct > 0:
                rec_lines.append(f"**HOLD / ADD** — Below SMA_200 but macro supports gold ({', '.join(macro_signals[:2])}).")
                ...
                rec_lines.append(f"**ADD (careful)** — Down {abs(pnl_pct):.1f}%, macro supports gold. Scale in at ${nearest_support:.0f}.")
            ...
            if macro_favors_long:
                rec_lines.append(f"**HOLD / ADD** — Macro supports gold. Buy dips to ${nearest_support:.0f}.")
                rec_lines.append(f"   Fundamentals say gold goes higher. Accumulate cheaply.")
            elif macro_favors_short:
                rec_lines.append(f"**HOLD** — Mixed signals, macro cautious. Stop at ${hard_stop:.0f}.")
                rec_lines.append(f"   Gold goes up long-term — no rush to add at uncertain levels.")
            else:
                if pnl_pct > 3:
                    ...
                    rec_lines.append(f"   Gold goes up long-term. This drawdown is noise.")
        ...
        if risk_events_list:
            ...
            for e in risk_events_list[:5]:
                ...
                # Color = directional impact on gold price, not event severity
                bias = e.get("gold_bias", "uncertain")
                if bias == "bearish":
                    bias_icon = "🔴"
                elif bias == "bullish":
                    bias_icon = "🟢"
        ...
        # ---- CATALYST PROBABILITY / LEADING INDICATORS ----
        if catalyst_layer.get("available"):
            lines.append("")
            lines.append("**🧭 GOLD CATALYST LAYER:**")
            cat_score = catalyst_layer.get("score", 0)
            cat_icon = "🟢" if cat_score > 0 else ("🔴" if cat_score < 0 else "🟡")
            lines.append(f"   {cat_icon} {catalyst_layer.get('bias', 'NO CLEAR EVENT EDGE')} (score {cat_score:+d})")
            nxt = catalyst_layer.get("next_event")
            if nxt:
                event_pt = nxt["date"].astimezone(PT).strftime("%a %-m/%-d %-I:%M%p PT")
                lines.append(f"   Next catalyst: {nxt['event']} — {event_pt} ({nxt.get('hours_away', 0):.0f}h away)")
            for reason in catalyst_layer.get("reasons", [])[:4]:
                lines.append(f"   • {reason}")
            labor_read = catalyst_layer.get("labor", {})
            if labor_read.get("available"):
                lines.append(f"   Labor proxies: {labor_read.get('bias')} (labor score {labor_read.get('score', 0):+d})")
            pm = catalyst_layer.get("prediction_markets", {})
            kalshi = (pm.get("kalshi") or {})
            if kalshi.get("markets"):
                k = kalshi["markets"][0]
                prob = f" — {k.get('yes_pct')}% Yes" if k.get("yes_pct") is not None else ""
                lines.append(f"   Kalshi: {k.get('title', k.get('ticker'))}{prob}")
            elif kalshi.get("note"):
                lines.append(f"   Kalshi: {kalshi.get('note')}")
            if pm.get("polymarket"):
                p = pm["polymarket"][0]
                lines.append(f"   Polymarket: {p.get('question')} — {p.get('yes_pct', '?')}% Yes")
        ...
        if pnl_pct < -0.5 and current > hard_stop:
            ...
            if not structural_break:
                rules.append(f"   Gold goes up long-term. Selling on a dip in a bull market is how you lose money.")
        ...
        if macro_favors_long and tech_sub <= -3:
            rules.append(f"💡 Chart says sell but macro supports gold ({', '.join(macro_signals[:2])}). Don't panic sell — macro wins long-term.")
        ...
        # Hard stop
        if contracts:
            hard_dollar = round((current - hard_stop) * 10 * contracts, 0)
            rules.append(f"Hard stop: **${hard_stop:.0f}** (${hard_dollar:,.0f} risk) — absolute floor, do not go below.")
            if take_profit_level:
                tp_dollar = round((take_profit_level - entry_price) * 10 * contracts, 0)
    ...
    if macro:
        ...
        if macro.get("MINER_LEVERAGE"):
            lines.append(f"   Miner Leverage: {macro['MINER_LEVERAGE']}x (GDX/GLD)")
        if macro.get("GSR"):
            lines.append(f"   Gold/Silver Ratio: {macro['GSR']} — {macro.get('GSR_SIGNAL', '')}")
        if macro.get("FEDWATCH"):
            fw = macro["FEDWATCH"]
            path = macro.get("FEDWATCH_PATH", {})
            next_mtg = macro.get("FEDWATCH_NEXT", {})
            parts = []
            if next_mtg.get("date"):
                parts.append(f"next {next_mtg['date']}: {next_mtg.get('implied_rate', '?')}% implied")
            if path.get("through_date"):
                far_meeting = (fw.get("meetings") or [{}])[-1]
                if fw.get("target_source") == "front Fed Funds futures proxy":
                    parts.append(f"{far_meeting.get('implied_rate', '?')}% by {path['through_date']}")
                else:
                    cuts = path.get("cuts_priced", 0) or 0
                    hikes = path.get("hikes_priced", 0) or 0
                    if cuts:
                        parts.append(f"{cuts:.1f} cuts priced by {path['through_date']}")
                    elif hikes:
                        parts.append(f"{hikes:.1f} hikes priced by {path['through_date']}")
            repricing = fw.get("avg_repricing_1w_bp")
            repricing_str = f", {repricing:+.1f}bp 1w" if repricing is not None else ""
            detail = f" ({'; '.join(parts)})" if parts else ""
            lines.append(f"   FedWatch/Fed Funds: {fw.get('signal', 'NEUTRAL')}{repricing_str}{detail}")
    ...
    if options.get("put_call_oi_ratio"):
        ...
        lines.append(f"   GLD P/C OI: {oi}x ({signals.get(options['signal'], options['signal'])})")
...
def format_levels_only(data):
    ...
    now = _et_now()
    now_pt = now.astimezone(PT).strftime("%-I:%M %p PT")
    lines.append(f"🥇 GOLD LEVELS — {now_pt}")
    
    if gold.get("price"):
        lines.append(f"Current: ${gold['price']}")
...
def main():
    parser = argparse.ArgumentParser(description="Gold Futures Trader Dashboard")
    ...
    # Separate OHLC fetch for gold futures so ATR is computed from H/L/C, not close-only
    try:
        gold_ohlc = yf.Ticker("GC=F").history(period="1y", interval="1d", auto_adjust=True)
        if gold_ohlc is None or len(gold_ohlc) == 0:
            gold_ohlc = None
    except Exception:
        gold_ohlc = None

    gold_series = price_data.get("GC=F", {}).get("series")
    technicals = compute_technicals(gold_series, gold_ohlc=gold_ohlc) if gold_series is not None else {"error": "No gold data"}

    # Clean series from output (not JSON-serializable)
    ...
    macro = compute_macro(price_data, real_yield=real_yield, fedwatch=fedwatch)
    volume = fetch_futures_volume()
    options = fetch_gld_options()
    ig = fetch_ig_sentiment()
    seasonal = seasonal_context()
    fed_news = fetch_fed_news()
    fed_sentiment = classify_fed_news_sentiment(fed_news)
    polymarket = fetch_polymarket_gold()
    kalshi = fetch_kalshi_macro_markets()
    labor = fetch_labor_leading_indicators()
    risk_events = get_risk_events()
    catalyst_layer = build_gold_catalyst_layer(risk_events, labor, macro, fedwatch, polymarket, kalshi)
    ...
    daily_close = price_data.get("GC=F", {}).get("price")
    intraday_price = fetch_intraday_last_price() if session.get("trading_active") else None
    gold_price = intraday_price if intraday_price else daily_close
    if intraday_price and price_data.get("GC=F"):
        price_data["GC=F"]["price"] = intraday_price
        price_data["GC=F"]["intraday"] = True
        price_data["GC=F"]["daily_close"] = daily_close
    ...
    levels = generate_levels(gold_price, technicals, conviction,
                              macro=macro, options=options, ig=ig) if gold_price else {"error": "No price"}
    ...
    ladder = generate_ladder(
        gold_price, args.entry_price, args.contracts, effective_max_contracts,
    ...
    ) if (gold_price and args.entry_price and args.contracts) else {"buy_ladder": [], "sell_ladder": [], "core_contracts": 0}
