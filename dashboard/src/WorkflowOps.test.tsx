...
describe('Workflow Ops evidence boundaries', () => {
  ...
  it('keeps the current consumers explicit and omits absent runtime metadata', () => {
    ...
    expect(html).toContain('Daily Brief market context')
    expect(html).toContain('Conviction List membership')
    expect(html).not.toContain('CIO supporting evidence')
    expect(html).not.toContain('CIO membership and supporting evidence')
  ...
  it('shows source/run/publication timestamps separately without inferring a date-only clock', () => {
    ...
    expect(html).toContain('2026-09-04 19:20:02 EDT')
    expect(html).toContain('2026-09-06 00:05:06 EDT')
    expect(html).toContain('2026-09-06 21:02:03 EDT')
