...
export function isMacroRegimeMeter(value: unknown): value is MacroRegimeMeterData {
  ...
  const safePath = (text: unknown) => typeof text === 'string' && /^(?![\s\S]*[\x00-\x20\x7f])(?!.*[Pp][Rr][Ii][Vv][Aa][Tt][Ee])(?!.*[Aa][Cc][Cc][Oo][Uu][Nn][Tt])(?!.*[Pp][Oo][Rr][Tt][Ff][Oo][Ll][Ii][Oo])(?!.*[Pp][Nn][Ll])(?!.*[Rr][Oo][Bb][Ii][Nn][Hh][Oo][Oo][Dd])(?!.*[Cc][Oo][Ss][Tt][_-]?[Bb][Aa][Ss][Ii][Ss])(?!.*[Dd][Rr][Yy][_-]?[Pp][Oo][Ww][Dd][Ee][Rr])(?!.*[Gg][Oo][Aa][Ll][_-]?[Gg][Aa][Pp])(?!.*\.\.)[A-Za-z0-9_-]+(?:[.][A-Za-z0-9_-]+)*(?:\/[A-Za-z0-9_-]+(?:[.][A-Za-z0-9_-]+)*)*$/.test(text) && !/[\r\n]/.test(text)
